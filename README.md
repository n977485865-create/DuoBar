...
5. **专注圆点**：点击“连接专注模式”，进入 **系统设置 → 专注模式 → 勿扰模式 → 添加过滤条件**，选择 **DuoBar**，把“专注圆点”设为 **点亮**，然后添加。此后随这个模式自动点亮和熄灭。工作、睡眠等模式需要各自添加一次；没有添加的模式不会联动。不需要通知权限、共享专注状态或 Apple 开发者账号。
...
此前实机验收为 Apple Silicon / macOS 26.6.2，包含菜单交互、圆点设置与重启保留、状态读取、系统设置跳转及登录项注册/取消。当前专注过滤条件的系统注册与完整开关切换正在实机验收。实际注销登录、睡眠唤醒、Intel 和其他 macOS 版本尚未验收。
